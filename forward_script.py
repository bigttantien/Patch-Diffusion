...
from training import forward_function
...
@click.option('--network_pkl', 'network_pkl',  help='Network pickle filename', metavar='PATH|URL',                      type=str, required=True)
...
def forward(**kwargs):
    ...
    if dist.get_rank() == 0:
        ...
        with open(os.path.join(c.run_dir, 'forward_options.json'), 'wt') as f:
            json.dump(c, f, indent=2)
        dnnlib.util.Logger(file_name=os.path.join(c.run_dir, 'log.txt'), file_mode='a', should_flush=True)

    # Forward function.
    forward_function.forward_function(**c)
